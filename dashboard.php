...
$rankedStatus = null;
if (!$is_guest && function_exists('ranked_status')) {
  try {
    ranked_try_create_match($mysqli);
    $rankedStatus = ranked_status($mysqli, (int)$u['id']);
  } catch (Throwable $e) {
    $rankedStatus = null;
  }
}

$rankedQueue = is_array($rankedStatus) ? ($rankedStatus['queue'] ?? []) : [];
$rankedMatch = is_array($rankedStatus) ? ($rankedStatus['match'] ?? []) : [];
$rankedInQueue = !empty($rankedQueue['in_queue']);
$rankedMatchFound = !empty($rankedMatch['found']) && !empty($rankedMatch['room_code']);
$rankedJoinedAt = (string)($rankedQueue['joined_at'] ?? '');
$rankedLeague = (string)($rankedQueue['league'] ?? 'bronze');
$rankedTier = (string)($rankedQueue['tier'] ?? 'Bronze');
$rankedQueueCount = (int)($rankedQueue['queue_count'] ?? 0);
$rankedQueuePosition = (int)($rankedQueue['queue_position'] ?? 0);
$rankedRoomCode = (string)($rankedMatch['room_code'] ?? '');
...
<?php if (!$is_guest): ?>
  <div
    class="ranked-floating-queue <?= ($rankedInQueue || $rankedMatchFound) ? 'is-visible' : '' ?>"
    id="rankedFloatingQueue"
    data-joined-at="<?= h($rankedJoinedAt) ?>"
    data-room-code="<?= h($rankedRoomCode) ?>"
    aria-live="polite"
  >
    <div class="ranked-floating-queue__top">
      <div>
        <div class="ranked-floating-queue__eyebrow">Ranked Queue</div>
        <div class="ranked-floating-queue__title" id="rankedFloatingTitle">
          <?= $rankedMatchFound ? 'Match found!' : 'Finding players...' ?>
        </div>
      </div>

      <span class="pill status-pill--good" id="rankedFloatingTimer">00:00</span>
    </div>

    <div class="ranked-floating-queue__body">
      <div id="rankedFloatingMeta">
        <?php if ($rankedMatchFound): ?>
          Entering room <?= h($rankedRoomCode) ?>...
        <?php elseif ($rankedInQueue): ?>
          <?= h($rankedTier ?: ucfirst($rankedLeague)) ?> queue · <?= max(1, $rankedQueueCount) ?> / 4 players found
        <?php else: ?>
          Queue inactive.
        <?php endif; ?>
      </div>
    ...
    <div class="ranked-floating-queue__actions">
      <a class="btn btn-primary" id="rankedFloatingEnterBtn" href="<?= h($bp) ?>/room.php?code=<?= h($rankedRoomCode) ?>" <?= $rankedMatchFound ? '' : 'hidden' ?>>
        Enter Match
      </a>

      <button class="btn btn-ghost" id="rankedFloatingCancelBtn" type="button" <?= $rankedInQueue ? '' : 'hidden' ?>>
        Cancel Queue
      </button>
    </div>
  </div>
  ...
  (() => {
    ...
    const basePath = <?= json_encode($bp, JSON_UNESCAPED_SLASHES) ?>;
    const card = document.getElementById("rankedFloatingQueue");
    const title = document.getElementById("rankedFloatingTitle");
    const timer = document.getElementById("rankedFloatingTimer");
    const meta = document.getElementById("rankedFloatingMeta");
    const cancelBtn = document.getElementById("rankedFloatingCancelBtn");
    const enterBtn = document.getElementById("rankedFloatingEnterBtn");

    if (!card || !title || !timer || !meta || !cancelBtn || !enterBtn) return;

    let joinedAt = card.dataset.joinedAt || "";
    let roomCode = card.dataset.roomCode || "";
    let pollTimer = null;
    let clockTimer = null;
    ...
    function showQueue(status) {
      ...
      if (match.found && match.room_code) {
        roomCode = String(match.room_code);
        card.dataset.roomCode = roomCode;

        card.classList.add("is-visible");
        title.textContent = "Match found!";
        meta.textContent = "Entering room " + roomCode + "...";
        cancelBtn.hidden = true;
        enterBtn.hidden = false;
        enterBtn.href = basePath + "/room.php?code=" + encodeURIComponent(roomCode);
      ...
      if (queue.in_queue) {
        ...
        card.dataset.joinedAt = joinedAt;

        const count = Math.max(1, Number(queue.queue_count || 1));
        const tier = queue.tier || queue.league || "Ranked";

        card.classList.add("is-visible");
        title.textContent = "Finding players...";
        meta.textContent = tier + " queue · " + Math.min(count, 4) + " / 4 players found";
